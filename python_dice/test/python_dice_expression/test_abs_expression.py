...
class TestAbsExpression(unittest.TestCase):
    def setUp(self):
        ...
        self._test_binary_operator = abs_expression.AbsExpression(self._mock_syntax)
    ...
    def test_abs_roll_pos(self):
        for _ in range(100):
            self.assertEqual(2, self._test_binary_operator.roll())

    def test_abs_roll_neg(self):
        self._mock_syntax.roll.return_value = -4
        for _ in range(100):
            self.assertEqual(4, self._test_binary_operator.roll())

    def test_abs_max(self):
        self.assertEqual(5, self._test_binary_operator.max())

    def test_abs_min(self):
        self.assertEqual(1, self._test_binary_operator.min())

    def test_abs_str(self):
        self.assertEqual("ABS(7d3)", str(self._test_binary_operator))

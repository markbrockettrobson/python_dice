...
class TestBinaryOperatorGreaterThanExpression(unittest.TestCase):
    def setUp(self):
        ...
        self._mock_syntax[0].get_probability_distribution.return_value = ProbabilityDistribution({-3: 1, 0: 2, 4: 1})
        ...
        self._mock_syntax[1].get_probability_distribution.return_value = ProbabilityDistribution(
            {-3: 2, 0: 1, 1: 2, 4: 1}
        )
    ...
    def test_binary_operator_add_production_function(self):
        BinaryOperatorExpression.add_production_function(self._mock_parser_gen)
    ...
    def test_gt_max_false(self):
        self._mock_syntax[0].get_probability_distribution.return_value = ProbabilityDistribution({5: 1})
        self._mock_syntax[1].get_probability_distribution.return_value = ProbabilityDistribution({5: 1})

        self.assertEqual(0, self._test_binary_operator.max())

    def test_gt_min_true(self):
        self._mock_syntax[0].get_probability_distribution.return_value = ProbabilityDistribution({5: 1, 6: 1})
        self._mock_syntax[1].get_probability_distribution.return_value = ProbabilityDistribution({4: 1})

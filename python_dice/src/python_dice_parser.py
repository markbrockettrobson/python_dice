...
import python_dice.src.probability_distribution.probability_distribution_state as probability_state
...
class PythonDiceParser(i_pydice_parser.IPythonDiceParser):
    def __init__(self, lexer: i_pydice_lexer.IPythonDiceLexer = None):
        ...
        def error_handler(_: probability_state.ProbabilityDistributionState, token: rply.token.Token):
            raise ValueError(
    ...
    def parse(
        ...
        if state is None:
            state = probability_state.ProbabilityDistributionState()
